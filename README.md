...
### ⚡ 核心优势与特色
* **极致降本**：注册即赠送 50 积分，生成单条视频仅需 10 积分（10 积分 = 1 元），对比传统外包成本降低 98% 以上。
...
| 并发锁 | SQL 悲观锁 (`FOR UPDATE`) | 积分扣减存储过程 | 保证积分扣除高并发下的扣减准确 |
...
│   ├── pages/                     # 42 个核心业务页面
│   │   ├── LandingPage.tsx        # 品牌官网首页 (展示 1798+ 生成视频，169+ 商家)
│   │   ├── HomePage.tsx           # 视频生成工作台 (支持参考、数字人、首尾帧控制)
...
① 全球选品 / 商品 URL 解析 ➔ DeepSeek-V4-Flash 自动提炼三项爆款卖点与商品图
        │
        ▼
② 工作台配置 ➔ 选择【参考 ➔ 数字人 ➔ 首尾帧】，AI 提示词增强（带粒度进度条动效）
        │
        ▼
...
### 4. 💰 高颜值收银台与积分结算
* 注册即送 **50 积分**，生成单条视频扣除 **10 积分**（10 积分 = 1 元）。
