...
function PhoneSmsPanel() {
  ...
  throw new Error(data?.message || '验证码错误或已过期');
}
} catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <Label className="text-sm font-normal">手机号</Label>
      ...
      {codeSent && (
        <div className="space-y-1.5">
          <Label className="text-sm font-normal">验证码</Label>
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
        />
        <Button type="button" variant="outline" disabled={countdown > 0 || sending} onClick={handleSend} className="shrink-0 w-28">
      ...
      {/* 发送/验证按钮 */}
      {!codeSent ? (
        <Button className="w-full h-11 font-semibold" onClick={handleSend} disabled={sending}>
          {sending ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />发送中...</span> : '获取验证码'}
    </Button>
) : (
  <Button className="w-full h-11 font-semibold" onClick={handleVerify} disabled={verifying}>
...
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 rounded-full pointer-events-none"
  style={{ background: 'radial-gradient(circle, rgba(139,92,246,0.12) 0%, transparent 70%)' }} />
    ...
    <div>
      <p className="font-bold text-white text-lg leading-none tracking-wide">AIGC 带货</p>
      <p className="text-white/50 text-xs mt-0.5">电商视频生成平台</p>
</div>
</div>
      ...
      <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full border"
        ...
        <h1 className="text-5xl font-black text-white leading-[1.1] text-balance" style={{ letterSpacing: '-0.02em' }}>
          带货视频<br />
          <span style={{
...
<p className="text-white/55 text-base leading-relaxed max-w-xs text-pretty">
