...
import {
  Sparkles, ChevronDown, ImageIcon, Video, Wand2, Shirt,
  BarChart2, Captions, Droplets, ArrowUpCircle, Mic, Globe, RefreshCcw,
  MoreHorizontal, Maximize2, Copy, Plus, ChevronRight,
} from 'lucide-react';
...
const MODELS = ['Wan 2.7', 'Kling 2.1', 'Sora 2', 'Runway'];
...
export default function HomePage() {
  ...
  const [model, setModel] = useState('Wan 2.7');
    ...
    >
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-10 space-y-12">
                    ...
                    <div className="absolute top-full mt-1 left-0 z-50 bg-[#1e1d2a] border border-white/10 rounded-xl shadow-2xl py-1 min-w-[140px]">
                      {MODELS.map(m => (
                        <button key={m} onClick={() => { setModel(m); setModelOpen(false); }}
                          className={cn('w-full text-left px-3 py-2 text-xs hover:bg-white/10 transition-colors', m === model ? 'text-emerald-400' : 'text-white/70')}>
                          {m}
                        </button>
                      ))}
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          {/* 左侧大卡：图片生成 */}
          <div
            className="lg:col-span-2 rounded-2xl p-6 flex flex-col justify-between min-h-[220px] relative overflow-hidden cursor-pointer group"
          ...
          >
            <img
              src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&h=280&fit=crop"
              alt="图片生成"
            ...
            <button
              className="relative z-10 w-fit mt-4 px-5 py-2 rounded-full text-sm font-semibold text-white transition-all hover:scale-105"
        ...
        {/* ── 工具条 ────────────────────────────────────────────────── */}
        <div
          className="grid grid-cols-3 md:grid-cols-6 rounded-2xl overflow-hidden divide-x divide-white/6"
            ...
            {INSPIRE_IMGS.map((src, i) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button className="w-8 h-8 rounded-full bg-white/20 backdrop-blur flex items-center justify-center hover:bg-white/30 transition-colors" onClick={() => navigate('/video/create')}>
