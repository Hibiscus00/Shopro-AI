...
#### 🛠️ 解决方案（核心功能与工作流）
1.  **URL 一键提取卖点**：输入商品详情页 URL，系统自动抓取内容并利用 **DeepSeek-V4-Pro** 梳理出 3 条核心卖点。
...
graph TD
    ...
    Edge -->|API Calls| LLM[模型层: DeepSeek-V4-Pro / MiniMax-M3]
...
1.  **商品卖点智能提取**：用户提供商品 URL，Edge Function 抓取 HTML 文本，发送给 **DeepSeek-V4-Pro** 提取结构化卖点。
...
| :--- | :--- | :--- | :--- |
| **DeepSeek-V4-Pro (已对接)** | URL 卖点提炼、多语种口播脚本翻译、商品文本清洗、核心卖点挖掘、四层分镜脚本生成以及 NLP 情感分析。 | **输入**：网页原始文本/HTML 或商品卖点参数<br>**输出**：精炼的卖点、结构化脚本分镜或情感分析 JSON | 行业领先的多模态文本与脚本生成能力，在电商语境及文案逻辑处理上表现卓越，响应极速且极具性价比。 |
| **StepAudio 2.5 (已对接)**<br>(`stepaudio-2.5-asr` / `-tts`) | **ASR 录音转写**：支持用户语音录入与流式 ITN 转录。<br>**TTS 情感配音**：为生成好的脚本合成本土语种配音并输出高保真音频。 | **输入**：转写音频流 (ASR) 或口播台词 (TTS)<br>**输出**：识别的中文/英文文本或多语种情感音频 (WAV/MP3) | 专属多模态语音大模型，能够实现极其逼真、富有表现力的情感化带货口播合成，且 ASR 转写准确率高。 |
...
```mermaid
graph TD
    A[用户输入 URL/商品信息] -->|DeepSeek-V4-Pro| B[网页信息清洗 & 提炼 3 大卖点]
    B -->|DeepSeek-V4-Pro| C[结合卖点分析潜在受众画像与痛点]
    C -->|DeepSeek-V4-Pro| D[融合画像与卖点进行深度营销概念重组]
    D -->|DeepSeek-V4-Pro| E[基于四层 Agent 编排输出结构化脚本]
